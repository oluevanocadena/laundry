...
export class OrdersDraftFacade extends FacadeBase {
  ...
  discount = new FormProp<number>(this.formDiscount, 'discount', 0);
  discountType = new FormProp<DiscountTypes>(this.formDiscount, 'discountType', 'amount');
  ...
  submitForm() {
    ...
    this.repo.save(order, orderItems).then((response) => {
  ...
  updateOrderItemStatus(status: OrderItemStatusEnum) {
    ...
    if (orderItemToSave && orderItemToSave.id) {
      //Update single item
      this.repo.updateOrderItemStatus(orderItemToSave.id, status).then((response) => {
        if (response.success) {
          this.selectedOrder.value = response.data as unknown as Order;
      ...
      //Update all items
      this.repo.updateAllOrderItemsStatus(this.order.value?.id ?? '', status).then((response) => {
        if (response.success) {
          this.order.value = response.data as unknown as Order;
  ...
  onCancelOrder() {
    if (this.order.value?.id) {
      try {
        this.repo.updateOrderStatus(this.order.value?.id, OrderStatusEnum.Cancelled).then((orderSaved) => {
          if (orderSaved) {
            this.showCancelOrderModal = false;
            this.router.navigate([routes.Orders]);
          }
